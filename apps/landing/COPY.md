...
- Lead with a task someone can send: compare vendors, check a dashboard, fix a
  failing test. Follow with how the agent does it and what the user can review.
- Prefer concrete verbs: message, browse, check, save, run, review, connect.
- Explain features through their use. A morning dashboard check makes a
  schedule easier to understand; a shared brief explains files across agents.
...
- Skills save reusable instructions and supporting files. Do not imply that an
  agent learns workflows by watching unless that behavior is implemented.
- Describe memory through saved conversations, notes, and browser profiles.
