...
test('host shell completions survive restarts and retry delivery without re-polling settled jobs', async () => {
 ...
 try {
  await first.register({ botId: 'bot', machineId: 'machine', shellId: '123', outputPath: '/tmp/123.log' }); await first.flush(); first.dispose();
  expect(polls).toBe(1); expect(deliveries).toBe(1);
  const second = new HostShellCompletions(directory, async () => { throw new Error('A settled job must not be polled again'); }, async (receipt) => { expect(receipt).toMatchObject({ scope: 'bot', machineId: 'machine', hostShellId: '123', exitCode: 0 }); deliveries++; });
