...
Read [OpenTeam execution notes](../../OPENTEAM.md) before this workflow.

# Create Slack App

Help the developer create a Slack app or agent using Bolt for the requested framework.
...
- **If the requested variant is `bolt-js`**: Run `node --version` to verify Node.js is installed (v18+ required). If not installed, suggest `brew install node` or point to <https://nodejs.org>.
- **If the requested variant is `bolt-python`**: Run `python3 --version` to verify Python is installed (3.6+ required). If not installed, suggest `brew install python3` or point to <https://python.org>.
...
The app needs a Slack workspace to install into. Three targets work and they are not equivalent. Use the available user-question tool to let the developer pick, presenting them in this order:
...
- **If no sandbox exists**: ask only for its non-secret name and preferred target. Create it through the Slack Developer Program browser flow at <https://api.slack.com/developer-program/sandboxes>. Use the available password manager or let the user enter the password directly in Slack. Never request passwords in chat or put them in shell command arguments. Verify the new sandbox with `SLACK_CMD sandbox list --team <team_id>`.
...
Ask the developer what kind of app they want to build. Present the available templates based on their chosen framework (the requested variant):
...
Use the available user-question tool to let the developer pick a template. Recommend the **Starter Template** for first-timers or the **Starter Agent** if they want to build an AI agent.

### 4a. Choose an AI provider (agent templates only)

If the developer picks **Starter Agent** or **Support Agent**, these templates contain subdirectories for different AI providers. Ask the developer which provider they want to use via the available user-question tool:
...
Agent templates need the selected provider's API key: `ANTHROPIC_API_KEY` for the Claude template, `OPENAI_API_KEY` for the OpenAI template, and the chosen backend's key for Pydantic AI.

Use an existing authorized secret-manager integration or the provider/Slack configuration UI. Never ask for an API key in chat, put its value in generated source or command arguments, or print it in logs. Read `SLACK_CMD env set --help` to determine whether the installed CLI provides a secure input method. If it does not, the user must enter it directly in their local terminal or provider UI. Verify only that the required variable names are configured; do not retrieve their values. This CLI environment is separate from the OAuth connection granted to the Bot.
...
- Create sandboxes through the provider UI as described in Step 3; never pass a password through CLI arguments. Use `SLACK_CMD sandbox list --team <team_id>` to verify the intended team without an interactive team selector.
