let renderVersion=0,lastHeight=0,mathTokens=[];
...
function reportHeight(){const h=Math.ceil(document.getElementById('message').getBoundingClientRect().height);if(h!==lastHeight){lastHeight=h;window.webkit?.messageHandlers?.height?.postMessage(h)}}
new ResizeObserver(reportHeight).observe(document.getElementById('message'));
window.renderMessage=async function(source,dark,fontSize,colors){
  ...
  mermaid.initialize({startOnLoad:false,securityLevel:'strict',theme:'base',themeVariables:{darkMode:dark,background:'transparent',primaryColor:colors.code,primaryTextColor:colors.text,primaryBorderColor:colors.muted,secondaryColor:colors.code,tertiaryColor:colors.code,lineColor:colors.muted,textColor:colors.text},maxTextSize:50000,htmlLabels:false,flowchart:{htmlLabels:false},suppressErrorRendering:true});
  let index=0;
