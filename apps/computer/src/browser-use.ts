...
import type { Browser, BrowserContext, BrowserType, Frame, Locator, Page } from "playwright-core";
...
const viewId = { type: "string", minLength: 1, maxLength: 120 } as const;
const element = { type: "string", maxLength: 500 } as const;

export const BROWSER_USE_TOOLS: readonly BrowserUseToolDefinition[] = [
  {
    name: "browser_navigate",
    description:
      "Navigate the worker browser to a URL. Reuses the selected tab unless newTab is true and returns page state with a screenshot.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Capture the current page structure with [ref=eN] handles. Refs belong to this tab's latest snapshot and must be refreshed after page changes.",
    inputSchema: {
      type: "object",
      properties: {
        viewId,
  ...
  {
    name: "browser_click",
    description: "Click an element by ref from browser_snapshot and return the resulting page.",
    inputSchema: {
      type: "object",
      properties: {
        ref: { type: "string" },
...
export class BrowserUseSession {
  ...
  private readonly refs = new Map<string, Map<string, Locator>>();
  ...
  private async ensurePage(requestedViewId?: string): Promise<Page> {
    ...
    for (const page of pages) this.idFor(page);
    if (requestedViewId) {
    ...
    const selected = pages.find((page) => this.idFor(page) === this.currentViewId) ?? pages.at(-1);
    if (selected) {
  ...
  private clearRefs(page: Page): void {
    this.refs.delete(this.idFor(page));
  }

  private async requireRef(page: Page, value: unknown): Promise<Locator> {
    if (typeof value !== "string") throw new Error("An element ref is required");
    const locator = this.refs.get(this.idFor(page))?.get(value);
    if (!locator) {
  ...
  private async snapshot(args: JsonObject) {
    ...
    if (args.interactive !== true) {
      const bodyText = await page
        .locator(typeof args.selector === "string" ? args.selector : "body")
        .first()
        .innerText({ timeout: 5_000 })
        .catch(() => "");
      if (bodyText.trim()) lines.push(`Visible text:\n${bodyText.trim().slice(0, 20_000)}`);
    }
    let nextRef = 1;
    const selector =
    ...
    for (const frame of page.frames()) {
      const frameEntries = await this.snapshotFrame(frame, selector, refs, nextRef);
  ...
  private async snapshotFrame(
    ...
    const count = Math.min(await candidates.count().catch(() => 0), 500 - startRef + 1);
  ...
  private async click(args: JsonObject) {
    const page = await this.ensurePage(this.viewId(args));
    const locator = await this.requireRef(page, args.ref);
    ...
    if (typeof args.holdDurationMs === "number" && args.holdDurationMs > 0) {
      ...
      await locator.click({
        ...
        ...(typeof args.offsetX === "number" && typeof args.offsetY === "number"
          ? { position: { x: args.offsetX, y: args.offsetY } }
          : {}),
      });
    }
    this.clearRefs(page);
  ...
  private async type(args: JsonObject) {
    ...
    if (args.clear === true) await locator.fill("");
    if (args.slowly === true) await locator.pressSequentially(args.text, { delay: 40 });
    else await locator.pressSequentially(args.text);
  ...
  private async selectOption(args: JsonObject) {
    ...
    let selected = await locator.selectOption(args.values as string[]).catch(() => []);
    if (selected.length === 0) {
      selected = await locator.selectOption((args.values as string[]).map((label) => ({ label })));
    }
    this.clearRefs(page);
  ...
  private async drag(args: JsonObject) {
    ...
    if (typeof args.targetRef === "string") {
      await source.dragTo(await this.requireRef(page, args.targetRef));
    } else if (typeof args.targetX === "number" && typeof args.targetY === "number") {
      const box = await source.boundingBox();
      if (!box) throw new Error("Source element has no visible bounding box");
      await page.mouse.move(box.x + box.width / 2, box.y + box.height / 2);
      await page.mouse.down();
      await page.mouse.move(args.targetX, args.targetY, { steps: 8 });
      await page.mouse.up();
    } else {
  ...
  private async highlight(args: JsonObject) {
    const page = await this.ensurePage(this.viewId(args));
    const locator = await this.requireRef(page, args.ref);
    const original = await locator.evaluate((node) => {
      const element = node as HTMLElement;
