...
import type { LoadedChannelHistory } from "./history";
import {
...
export interface ChannelHistoryWindowStatus {
  beforeSequence: string | null;
  hasMore: boolean;
  hasNewer: boolean;
}

/** Small headless store for clients that publish one snapshot rather than separate history lanes. */
export const createChannelHistoryStore = () => {
  const entries = new Map<string, Pick<MessageWindowTransition, "history" | "window">>();
  const projections = new Map<
    string,
    {
      visible?: ChannelMessageView[];
      retained?: ChannelMessageView[];
      visibleIds?: ReadonlySet<string>;
      status?: ChannelHistoryWindowStatus;
    }
  >();
  const projectionFor = (channelId: string) => {
    let projection = projections.get(channelId);
    if (!projection) {
      projection = {};
      projections.set(channelId, projection);
    }
    return projection;
  };
  const viewports = new Map<string, { atBottom: boolean; viewport: MessageViewportRetention }>();
  const save = (channelId: string, next: Pick<MessageWindowTransition, "history" | "window">) => {
    entries.set(channelId, next);
    projections.delete(channelId);
    return next;
  };
  const visible = (channelId: string): ChannelMessageView[] | null => {
    const entry = entries.get(channelId);
    if (!entry) return null;
    return (projectionFor(channelId).visible ??= visibleChannelHistoryMessages(
      entry.history,
      entry.window
    ));
  };
  return {
    get: (channelId: string) => entries.get(channelId),
    visible,
    visibleIds: (channelId: string): ReadonlySet<string> | null => {
      const messages = visible(channelId);
      return messages
        ? (projectionFor(channelId).visibleIds ??= new Set(messages.map((message) => message.id)))
        : null;
    },
    retained: (channelId: string): ChannelMessageView[] | null => {
      const entry = entries.get(channelId);
      if (!entry) return null;
      return (projectionFor(channelId).retained ??= [
        ...uniqueRetainedMessages(
          entry.history.messages,
          entry.history.threadContext,
          entry.history.searchContext,
          entry.history.searchThreadContext,
          ...latestTailLanes(entry.window.latestTail)
        ).values(),
      ].sort(compareEntitySequence));
    },
    status: (channelId: string): ChannelHistoryWindowStatus | null => {
      const entry = entries.get(channelId);
      if (!entry) return null;
      const { history, window } = entry;
      return (projectionFor(channelId).status ??= {
        beforeSequence: window.context
          ? (history.searchContext[0]?.sequence ?? null)
          : history.beforeSequence,
        hasMore: window.context ? window.context.hasMoreBefore : history.hasMore,
        hasNewer: window.context ? window.context.hasMoreAfter : window.primaryHasNewerGap,
      });
    },
    setViewport: (channelId: string, messageIds: readonly string[], atBottom: boolean) => {
      viewports.set(channelId, {
        atBottom,
        viewport: {
          messageIds: new Set(messageIds),
          fill: atBottom ? "newer-first" : "older-first",
        },
      });
    },
    acceptPage: (page: ChannelHistoryPage, mode: "replace" | "refresh" | "older") => {
      let current = entries.get(page.channelId);
      const position = viewports.get(page.channelId);
      // Once the native list has reached the actual live edge, leave context mode
      // before refreshing so newly appended messages keep following the viewport.
      if (
        mode === "refresh" &&
        position?.atBottom &&
        current?.window.context &&
        !current.window.context.hasMoreAfter
      ) {
        current = resetToLatestTail(current.history, current.window) ?? current;
      }
      return save(
        page.channelId,
        applyPrimaryHistoryPage({
          ...
          page,
          mode,
          atBottom: position?.atBottom ?? true,
          viewport: position?.viewport,
        })
      );
    },
    acceptContext: (page: ChannelMessageContextView) => {
      const current = entries.get(page.channelId);
      if (!current) return;
      if (current.history.messages.some((message) => message.id === page.targetMessageId)) return;
      save(page.channelId, enterMessageContext(current.history, current.window, page));
    },
    expand: (page: ChannelMessageContextView, direction: MessageHistoryDirection) => {
      let current = entries.get(page.channelId);
      if (!current) return;
      if (!current.window.context) {
        // A primary window that evicted newer rows can page forward through the
        // same context endpoint without adding a different navigation UI.
        current = enterMessageContext(current.history, current.window, {
          ...page,
          targetMessageId: current.history.messages.at(-1)?.id ?? page.targetMessageId,
          messages: current.history.messages,
          threadContext: current.history.threadContext,
          threadContextTruncated: current.history.threadContextTruncated,
          hasMoreBefore: current.history.hasMore,
          hasMoreAfter: current.window.primaryHasNewerGap,
        });
      }
      save(
        page.channelId,
        expandMessageContext({
          ...
          page,
          direction,
          viewport: viewports.get(page.channelId)?.viewport,
        })
      );
    },
    jumpToLatest: (channelId: string): boolean => {
      const current = entries.get(channelId);
      if (!current) return false;
      const next = resetToLatestTail(current.history, current.window);
      if (!next) return false;
      save(channelId, next);
      return true;
    },
    patch: (message: ChannelMessageView) => {
      const current = entries.get(message.channelId);
      const next =
        current &&
        patchRetainedMessageWindow(
          ...
          current.window,
          message,
          viewports.get(message.channelId)?.viewport
        );
      if (next) save(message.channelId, next);
    },
    delete: (channelId: string) => {
      entries.delete(channelId);
      projections.delete(channelId);
      viewports.delete(channelId);
    },
    clear: () => {
      entries.clear();
      projections.clear();
