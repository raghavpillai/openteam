...
- **WebFetch:** by default the built-in public HTTP(S) reader: DNS/IP checks and pinning on every redirect, charset decoding, HTML to Markdown with tables (plain text above 50,000 elements, so huge pages cannot stall the computer), text/JSON/feeds, PDF text, a 20 MiB bound and a file for oversized content. It retries once with browser-like headers when a site blocks it, returns a near-empty shell or drops the connection, and otherwise tells the agent to use the browser. Settings → Providers → Fetch can switch to Exa, Parallel or Firecrawl (crawling on that provider’s infrastructure) or turn fetch off. Each provider has a stored connection check. No authenticated browser context.
- **WebSearch:** chosen in Settings → Providers → Search: Exa, Brave Search, Parallel, Firecrawl, Bing (with a SerpApi key) or Perplexity, each with its own stored key and connection check; source links/snippets, bounded requests and cancellation. Search is off until chosen; the tool stays visible and fails with "Web search is not configured" instead of searching. See [web search and fetching](../configuration/web-search.md).
