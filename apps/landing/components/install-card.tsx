...
const steps = [
  ...
  [
    ...
    "Enter your server URL in the desktop app, sign in, and create an agent.",
  ],
] as const;
...
export function InstallCard() {
  ...
  return (
    <div
      className="ot-install-card"
      ref={cycle.ref}
      {...cycle.props}
      data-setup-stage={cycle.index}
    >
      <InstallCommand />
        ...
        {steps.map(([title, description], index) => (
          <li key={title} data-active={cycle.index === index} data-done={cycle.index > index}>
            <span className="ot-install-step-number" aria-hidden="true">
              {cycle.index > index ? <Check size={15} /> : `0${index + 1}`}
            </span>
            <div>
              <h3>{title}</h3>
              <p>{description}</p>
