...
public struct Message: Codable, Identifiable, Sendable, Hashable {
  ...
  public var date: Date? {
    MessageTimestampParser.shared.date(createdAt)
  }
}

/// History is projected again when a message arrives. Reuse both the parsers and
/// recent timestamp results instead of constructing thousands of formatters on
/// the UI thread. A fixed-size ring bounds storage; the lock also protects the
/// Foundation formatters when background decoding and UI projection overlap.
private final class MessageTimestampParser: @unchecked Sendable {
  static let shared = MessageTimestampParser()
  private struct Result { let date: Date? }
  private let lock = NSLock()
  private let standard = ISO8601DateFormatter()
  private let fractional = ISO8601DateFormatter.fractional
  private let internet = Date.ISO8601FormatStyle()
  private let milliseconds = Date.ISO8601FormatStyle(includingFractionalSeconds: true)
  ...
  func date(_ value: String) -> Date? {
    lock.lock()
    defer { lock.unlock() }
    if let result = results[value] { return result.date }
    // The server emits UTC ISO timestamps. Foundation's value-style parser
    // avoids ICU formatter work on the first load too. Keep the existing parser
    // for other representations, including older higher-precision timestamps.
    let fast: Date?
    switch value.utf8.count {
    case 24 where value.hasSuffix("Z"): fast = try? milliseconds.parse(value)
    case 20 where value.hasSuffix("Z"): fast = try? internet.parse(value)
    default: fast = nil
    }
    let date = fast ?? standard.date(from: value) ?? fractional.date(from: value)
    if keys.count == 2_048 {
      results.removeValue(forKey: keys[eviction])
      keys[eviction] = value
      eviction = (eviction + 1) % keys.count
    } else { keys.append(value) }
    results[value] = Result(date: date)
    return date
  }
}
