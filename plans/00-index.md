...
Status: active and reference plans only; completed plans removed after verification
...
This plan set records current architecture context, research evidence, and work that still has an open acceptance gate. Completed implementation plans are removed once the shipped code and tests reach parity.

## How future agents should read this plan set

Start with `30-canonical-context-handoff.md`. It records the current architecture, the evidence/instruction boundary, attachment provenance, and the precedence rules for resolving contradictions. Then read the active feature document that owns the subsystem being changed and verify every claim against code, migrations, tests, and the live Compose stack.

The remaining numbered files are not ordered by authority. Current code, migrations, tests, and live validation outrank planning prose. Closed implementation plans were deleted on 2026-09-01; their machine-readable and visual evidence remains under `plans/evidence/`.

## Decisions made for v0

1. The product has one owner account. Username/password sessions are required by default; authentication-disabled mode is an explicit trusted-network deployment option. There is no team or role model.
...
8. OpenBot owns the background summary/adoption policy over Pi's native context/session tree. Explicit curated durable state is a separate typed product layer; it does not replace compaction or dump transcripts into memory.
...
- `11-plugin-architecture-research.md`: implemented plugin foundation plus still-open catalog breadth, update/rollback, publishing, and sandbox follow-ups.
- `13-native-tool-surface.md`: shipped ten-tool surface plus still-open secure-request and full interactive-widget behavior.
