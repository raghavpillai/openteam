...
- OpenBot dynamic discovery and dispatch with per-turn discovery receipts, current-status checks, and nested argument validation; `openbot` exposes `Computer`, and `cursor` exposes `SendToAgent` plus the approved todo/orchestration/administration subset;
