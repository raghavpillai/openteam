...
- Docker, running. On macOS or Windows, install and open [Docker Desktop](https://www.docker.com/products/docker-desktop/). On Linux, install Docker Engine with the Compose plugin, version 2.20 or newer.
...
2. **Connect a model provider.** Sign in with ChatGPT or Claude, or paste an API key. You can skip this and run `openteam setup` later, but bots can't work until a provider is connected.

When you confirm, setup starts the server and prints your **server URL**. You'll need it in the next step. By default, the server is reachable from your local network or private VPN. To use it from anywhere, see [remote access](../configuration/remote-access.md).

## 3. Connect the desktop app

[Download the desktop app](https://openteam.so/download), open it, and enter your server URL. Sign in with the username and password you just created.

## 4. Give a bot a task

Choose **New chat**, then **Create new Bot**. The bot says hello and asks what it can help with. Reply with a small task:
