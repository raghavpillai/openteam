// @ts-nocheck
// Captured Grok Bot 0.47.0 form status receipts; no submitted values.
// ../packages/grok-bot-harness/src/runner/tools/sand-user-form.ts
  ...
  const lines2 = form.fields.map((field) => {
    ...
    if (outcome?.fillFailed === true) {
      if (heldForRemap.has(field.id)) {
        status = `NOT FILLED \u2014 ${failureKind !== void 0 ? (HELD_FIELD_MISS_BY_KIND[failureKind] ?? "the page moved or the visible target could not be filled") : "the page moved before this field was written"}. The host still HOLDS the submitted value: name its new target through ${SAND_REMAP_USER_FORM_TARGETS_TOOL_NAME} in the cursor dynamic namespace (see below)`;
        ...
        status = FILL_FAILED_STATUS_BY_KIND[failureKind] ?? FILL_FAILED_STATUS_UNCLASSIFIED;
      } else {
...
export { buildUserFormSubmittedAck, buildUserFormDismissedAck, buildUserFormRemapReceipt, buildUserFormUnfillableResult, buildUserFormDriverUnavailableResult, buildUserFormSkippedFieldsNote };
