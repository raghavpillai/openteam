...
const clientUpdateFailure = (status: OpenBotUpdateStatus): string => {
  const summary =
...
export default function UpdatesSettings() {
  ...
  const changeSshTarget = (value: string) => {
    ...
    if (normalized) localStorage.setItem(`openbot.update.ssh.${API_BASE}`, normalized);
    else localStorage.removeItem(`openbot.update.ssh.${API_BASE}`);
  };
  ...
  useEffect(() => window.openbot?.updates.onClientProgress(setClientUpdate), []);
  ...
  const newerDesktopReleaseAvailable =
    ...
    (compareOpenBotVersions(clientUpdate?.latestVersion ?? clientVersion, clientVersion) ?? 0) > 0;
  const serverTarget = useMemo(() => {
    ...
    return (compareOpenBotVersions(clientUpdate.latestVersion, server.currentVersion) ?? 0) > 0
  ...
  const check = async () => {
    ...
    try {
      const next = await window.openbot?.updates.check();
  ...
  const updateClient = async () => {
    ...
    try {
      ...
      if (next?.status !== "available") {
        next = (await window.openbot?.updates.check()) ?? next;
        if (next) setClientUpdate(next);
      }
      if (next?.status === "available") await window.openbot?.updates.openDownload();
  ...
  const updateServer = async () => {
    if (!serverNeedsUpdate || !window.openbot) return;
    setActionError(null);
    try {
  ...
  const copyServerCommand = async () => {
    ...
    try {
      await navigator.clipboard.writeText(
        serverTarget ? `openbot update --version ${serverTarget}` : "openbot update"
      );
      setCopied(true);
    ...
    compatibility === "server-update-required"
      ...
      : compatibility === "client-update-required"
        ...
        : compatibility === "unknown" && !serverLoading
          ? "OpenBot could not verify that the desktop and server releases match. Update the server first if it does not report a version."
          : null;
    ...
    <>
      ...
      <SettingsGroup>
        ...
        <SettingsRow
          ...
          description={
            serverUpdating
              ? (server.message ?? "Updating the managed OpenBot stack")
              : server?.currentVersion
                ? server.updaterAvailable
          ...
          <SettingsRow
            ...
            description={
              server?.updateMethod === "ssh"
                ? "Uses your operating system SSH agent and known_hosts; passwords are never stored. Keep OpenBot open until the update finishes"
