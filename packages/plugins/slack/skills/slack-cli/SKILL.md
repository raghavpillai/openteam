...
If 1b fails or returns a different value, ask the developer using the available user-question tool:
...
Slack auth is **per-team**, not a single boolean. Complete the login flow below whenever any of these is true:
...
### Complete login in Slack

Use `SLACK_CMD login --help` to inspect the installed CLI's supported login flow. When a secure interactive terminal or authorized computer session is available, drive the flow there, keeping its single-use ticket and challenge code out of chat, logs, and saved files. The user may need to approve it in the intended Slack workspace. Otherwise ask the user to complete `SLACK_CMD login` directly on the computer where the developer app will run; do not ask them to paste a challenge code into chat.

Verify the intended team with `SLACK_CMD auth list` afterward. A previous login to a different team is not sufficient. If the ticket expires, restart login instead of replaying the same ticket. Use `--team <team_id>` to select that workspace on subsequent commands. This developer CLI login is independent of OpenTeam's Slack MCP account.
...
- Interactive commands (e.g., `slack trigger create` without `--trigger-def`) need an interactive terminal. Use an authorized private terminal or let the developer run them directly there. Follow Step 5 for login; keep tickets and challenge codes out of chat and command arguments.
