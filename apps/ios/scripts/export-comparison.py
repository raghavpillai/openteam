...
page += ''.join(rows) + '<section><h2>Additional native checks</h2><div class="extras">' + ''.join(extras) + '</div></section><p>Release gates still open: native APNs, full plugin workspace, advanced rendering, complete computer gestures and signed physical-device validation. See apps/ios/docs/PARITY.md.</p></main></html>'
