...
import { communicationReminder, enrichUserInfo, promptFingerprint } from "../../src/runtime/prompt-context";

const user = { role: "user", content: "do work" };
const call = (name: string, args = {}) => ({ role: "assistant", content: [{ type: "toolCall", name, arguments: args }] });
...
test("runtime context describes actual tools and preserves saved skill catalog", () => {
  const text = enrichUserInfo("<user_info>\n<agent_skills>test skill</agent_skills>\n</user_info>", {
    cwd: "/workspace/task", transcriptPath: "/private/session.jsonl",
  ...
  expect(promptFingerprint("system", text, [], 2)).toEqual(promptFingerprint("system", text, [], 2));
  expect(promptFingerprint("system", text, ["Lookup"], 2).toolsSha).not.toBe(promptFingerprint("system", text, [], 2).toolsSha);
});
...
test("acknowledgement reminders fire during work, with silent/worker exclusions", () => {
  ...
  expect(communicationReminder([...messages, { role: "custom", customType: "openteam-communication-reminder" }], active)).toBeNull();
});
...
test("opening acknowledgement is followed by a result reminder after work", () => {
  ...
  expect(communicationReminder([user, call("SendToUser", { type: "text" }), call("Read")], active)?.kind).toBe("progress");
  expect(communicationReminder([user, call("SendToUser", { type: "widget" }), call("Read"), call("Read")], active)?.kind).toBe("ack");
});
