...
Usage includes the model request, including system instructions and tool definitions. Pi combines reported provider usage with estimates for appended messages. OpenTeam estimates the effective context when provider usage is unavailable or stale after compaction. This means the trigger is based on the best available token count, not an exact tokenizer measurement at every keystroke.
