...
Help the developer build a rich Block Kit layout. If the requested variant is provided, it specifies the target surface (`message`, `modal`, or `home-tab`).
...
Every block, element, and composition object is documented on `docs.slack.dev`. Append `.md` to any reference URL to fetch it as markdown with the available web-fetch tool (no auth required).

- **Master index**: the authoritative list of every block, block element, and composition object, each linking to its own page: `https://docs.slack.dev/reference/block-kit.md`. Fetch it with the available web-fetch tool to confirm a type exists and to get the link to its page.
...
1. Determine the surface from the requested variant or context
...
If the requested variant is provided and matches one of `message`, `modal`, or `home-tab`, use it directly.

Otherwise, ask the developer using the available user-question tool:
...
1. **Fetch only what you need** from the live docs:
   - Fetch the master index with the available web-fetch tool (`https://docs.slack.dev/reference/block-kit.md`) to confirm the block and element types you plan to use exist and to grab links to their pages.
...
POST to the endpoint with the **Shell tool**. The API uses form-urlencoded encoding, so pass the JSON directly as the parameter value.
