...
export const loadAdvancedMessagePlugins = (capabilities: AdvancedMessageCapabilities) => {
  ...
  const promise = (async () => {
    ...
    await Promise.all([
      capabilities.cjk
        ? import("./message-response-cjk").then((module) => {
            plugins.cjk = module.cjk;
          })
        : undefined,
      capabilities.code
        ? import("./message-response-code").then((module) => {
            plugins.code = module.code;
          })
        : undefined,
      capabilities.math
        ? import("./message-response-math").then((module) => {
            plugins.math = module.math;
          })
        : undefined,
      capabilities.mermaid
        ? import("./message-response-mermaid").then((module) => {
