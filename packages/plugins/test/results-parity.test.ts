...
test('Large tool results survive process replacement and are complete and account scoped',async()=>{
 ...
 try{
  const store=new ResultStore('provider','account-a',root), other=new ResultStore('provider','account-b',root);
  const original={file:'large',content:'Unicode 🌎 and control \u0000\n'.repeat(15000)};
  let page:any=await store.capture(original);expect(page.resultId).toBeTruthy();
  ...
  let assembled='';
  while(true){expect(JSON.stringify(page).length).toBeLessThan(75000);assembled+=page.jsonFragment;if(page.nextOffset===null)break;page=await replacement.page(page.resultId,page.nextOffset);}
...
test('The binary package validator accepts real model-sized base64 and rejects malformed payloads',()=>{
 ...
 definition.binaryFiles={'assets/model.bin':Buffer.alloc(8*1024*1024,1).toString('base64')};
 expect(parsePluginDefinition(definition).binaryFiles?.['assets/model.bin']).toBe(definition.binaryFiles['assets/model.bin']);
 for(const invalid of ['a===','abcd=','ab!d','ab=c'])expect(()=>parsePluginDefinition({...definition,binaryFiles:{'file.bin':invalid}})).toThrow('binary file');
});
...
test('Every advertised reference tool input remains discoverable with valid MCP schemas',()=>{
 ...
 for(const [provider,references] of Object.entries(referenceTools)){
  ...
  for(const reference of references){
   const actual=local.find(t=>t.name===reference.name);expect(actual).toBeTruthy();
   for(const key of Object.keys(reference.inputSchema.properties))expect(actual!.inputSchema.properties).toHaveProperty(key);
   expect(validator.getValidator(actual!.inputSchema)({unexpected:'field'}).valid).toBe(false);
  }
 }
});

test('Notion ships all 14 original workflow skills with their bodies and supporting provenance',()=>{
 const notion=pluginCatalog.find(p=>p.key==='notion')!;
 expect(notion.skills).toHaveLength(14);expect(notion.components).toEqual(['mcp','skills']);
 for(const skill of notion.skills){expect(skill.body.length).toBeGreaterThan(1000);expect(notion.files?.[`${skill.path}/SKILL.md`]).toContain('Account and tools');}
 expect(notion.files?.['README.md']).toContain('independently authored');
