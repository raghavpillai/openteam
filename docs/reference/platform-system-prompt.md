...
- **WebFetch:** by default the built-in public HTTP(S) reader: DNS/IP checks and pinning on every redirect, charset decoding, HTML to Markdown with tables, text/JSON/feeds, PDF text, a 20 MiB bound and a file for oversized content. It retries once with browser-like headers when a site blocks it or returns a near-empty shell, and otherwise tells the agent to use the browser. Settings → Providers → Fetch can switch to Exa, Parallel or Firecrawl (crawling on that provider’s infrastructure) or turn fetch off. Each provider has a stored connection check. No authenticated browser context.
