...
// Provisioning, foreground reads, and background transcript projection can all
// refresh one agent concurrently. Publish them in order so they cannot collide
// on the temporary file or replace a newer snapshot with an older one.
const pendingProjections = new Map<string, Promise<void>>();
...
export async function refreshDerivedProjections(
  ...
  const key = join(root, agentId);
  const previous = pendingProjections.get(key) ?? Promise.resolve();
  const current = previous
    .catch(() => undefined)
    .then(() =>
      withAgentStore(agentId, async () => {
  ...
  pendingProjections.set(key, current);
  try {
    await current;
  } finally {
    if (pendingProjections.get(key) === current) pendingProjections.delete(key);
  }
}
