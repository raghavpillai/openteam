...
If the requested variant is provided, it is either a full `method.name` (jump to Step 2 to read its contract) or a `family` name (go to Step 1 and find it in the index).
...
> - **State-changing or destructive methods** (`chat.postMessage`/`update`/`delete`, any `*.delete`/`*.remove`/`*.kick`/`*.archive`, and all `admin.*`) — prepare the exact action and confirm with the developer unless their current instructions already authorize it.
...
Fetch the live method index with the available web-fetch tool:
...
You do not need the CLI to call a method. Get a token of the type you determined above from the app's **OAuth & Permissions** page in the Slack app config (`https://api.slack.com/apps` → your app → **OAuth & Permissions** → **OAuth Tokens**): the **Bot User OAuth Token** (`xoxb-…`) or the **User OAuth Token** (`xoxp-…`). That page also lists the scopes currently granted — confirm the ones from Step 2 are present. Keep the token in authorized credential storage; never ask for it in chat or extract it from the MCP account or CLI session. Inject it into the process environment as `SLACK_API_TOKEN` without printing or saving its value. Then send it with curl or an SDK in Step 4 ("Via raw HTTP" / "Via an SDK").

---

## Step 4: Call the Method (Manage)

First apply the **execution posture**: if the method changes state (post/update/delete/archive/kick, or any `admin.*`), verify that the developer has authorized that exact action and destination; ask only if authorization is missing. Read-only calls can be run directly.
...
Use the **Shell tool** when the developer wants a raw request or isn't using the CLI. Send the separately authorized bot or user token from Step 3 in the `Authorization` header. The examples read the injected `SLACK_API_TOKEN` environment variable and pass the header through stdin, keeping its value out of curl arguments. Do not enable shell tracing or HTTP debug logs.
